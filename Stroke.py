...
def dict2CSV(data, year, option):
    keys = data[0].keys()
    with open('{}/{}_{}.csv'.format(NAME, YEAR_DICT[year], STROKE_DICT[option]), 'w', encoding="utf8") as f:
...
def checkExist(year, option):
    ...
    if not os.path.isdir(NAME):
        os.makedirs(NAME)
    if os.path.isfile("{}/{}_{}.csv".format(NAME, YEAR_DICT[year], STROKE_DICT[option])):
        return True
    return False
